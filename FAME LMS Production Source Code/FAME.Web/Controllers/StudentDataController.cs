...
namespace First_Aid_Made_Easy.Controllers
{
    public class StudentDataController : ApiController
    {
        ...
        [HttpGet]
        public IHttpActionResult IsRequestAccepted(string id)
        {
            var isAccepted = Common.IsRequestAccepted(id);
            return Ok(new { success = true, Data = isAccepted, message = isAccepted ? "Accepted" : "Not Accepted" });
        }
        [HttpGet]
        public IHttpActionResult MyCourses(string id)
        {
            var user = Common.GetUserNameByASpUserID(id);
            if (user != null)
            {
                MyCoursesVM MyCourses = _courseRepository.GetListByStudent(id);
                return Ok(new
                {
                    success = true,
                    data = new
                    {
                        Enrollment = _enrollmentRepository.GetCurrentEnroll(id),
        ...
        [HttpGet]
        public IHttpActionResult GetSections(string id, int CourseID)
        {
            var user = Common.GetUserNameByASpUserID(id);
            if (user != null)
            {
                CourseVM course = _courseRepository.GetByID(CourseID, id);
                course.SectionList = _sectionRepository.GetSectionsByCourse(CourseID, id);
                return Ok(new
                {
                    success = true,
                    data = course
        ...
        [HttpGet]
        public IHttpActionResult GetVideos(string id, int SectionID)
        {
            var user = Common.GetUserNameByASpUserID(id);
            if (user != null)
            {

                SectionVM s = _sectionRepository.GetByIDForTakeLec(SectionID, id);
                s.VideoList = _videoRepository.GetVideosBySection(SectionID, id);
